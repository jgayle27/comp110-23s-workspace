...
points: int
player: str
HORIZONTAL_PLANE: str = "\U0001F6E9"
SIDEWAYS_PLANE: str = "\U00002708"


def main() -> None:
    """Main function which initializes global variables."""
    global points
    global player
    points = randint(0, 100)
    greet()
    main_bool: bool = False
    # Present player with at least 3 options of where to go next and ask them to choose.
    while (not main_bool):
        option: str = input(f"\nYour current United Rewards points balance is {points}.\nHow can we help you today, {player}? \nPlease enter the corresponding number from the following options: \n (1) Schedule a flight \n (2) Use my points \n (3) Exit  \n")
        if (option == '1'):
            flight(player)
        if (option == '2'):
            points = rewards(points, player)
        if (option == '3'):
            print(f"\nSuccessfully logged out. \nYour remaining Rewards points balance is {points}. \n \n \n United Airlines\nGood leads the way. {HORIZONTAL_PLANE}")
            main_bool = True
...
def flight(name: str) -> None:
    """Procedure that gives player a choice between rates for flights."""
    global points
    flight_menu: bool = False
    while (not flight_menu):
        print(f"{SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}  {SIDEWAYS_PLANE}")
        print(f"United Flights \n \nWhere will you go, {name}?")
        player_origin: str = input("Enter city of origin:  ")
        player_destination: str = input("Enter destination city:  ")
        rates_low: int = randint(150, 300)
        rates_high: int = randint(301, 600)
        rates_choice: str = input(f"{player_origin} to {player_destination} rates calculated from ${rates_low} to ${rates_high}. \nSelect from these rates? Enter 'yes' or 'no'.  ")
        if (rates_choice == "yes"):
            points = rates_func(player_origin, player_destination, rates_low, rates_high, points)
            flight_menu = True
        else:
            ...
            if (return_main != 'main menu'):
                print("Please select a new origin and destination.\n")
            else:
                flight_menu = True


def rates_func(input_a: str, input_b: str, input_c: int, input_d: int, rates_points: int) -> int:
    ...
    rate_decision_bool = False
    option_num: int = 1
    weekdays: list[str] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    while (not rate_decision_bool):
        weekday_choice: str = weekdays[randint(0, 6)]
        flight_points: int = randint(15, 40)
        print(f"\n------\nFlight option {option_num}: {input_a} to {input_b}\nprice: ${randint(input_c, input_d)}\n")
        flight_decision: str = input("Select this flight? Enter 'yes' or 'no'  ")
...
def seat_upgrade(seat_points: int) -> int:
    ...
    if (flight_num <= 500):
        ...
        upgrade_choice: str = input("Use your points to upgrade seat? Enter 'yes' or 'no'.  ")
        if (upgrade_choice == 'yes'):
            if (seat_points < seat_amount):
                print("Your current rewards balance is unable to cover this upgrade. Redirecting you to the points selection menu.\n")
                return (seat_points)
...
def discount_flight(discount_points: int) -> int:
    ...
    if (flight_num <= 800):
        flight_discount: int = randint(0, 30)
        flight_cost: int = randint(150, 300)
...
def wifi(wifi_points: int) -> int:
    ...
    if (use_wifi == 'use points'):
        if (wifi_points == 0):
            print("Your current Rewards points balance is 0. Unfortunately you cannot purchase wifi at this time.\n----\nRedirecting you to the main menu.\n")
            return (wifi_points)
        else:
            if (wifi_points < 20):
                ...
                wifi_points = wifi_points - 20
                print(f"Your current Rewards points balance is {wifi_points}. Thank you for choosing United. We look forward to seeing you{SIDEWAYS_PLANE}\n----\nRedirecting you to the main menu.\n")
                return (wifi_points)
    else:        
        return (wifi_points)
